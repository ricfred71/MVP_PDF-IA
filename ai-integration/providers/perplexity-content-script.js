(() => {
  const log = (...args) => console.log('[PerplexityContentScript]', ...args);
  ...
  function trySetValue(el, value) {
    ...
    // Para contenteditable (Lexical Editor)
    if (el.getAttribute?.('contenteditable') === 'true' || el.isContentEditable) {
      try {
        // Limpa o campo
        el.textContent = '';
        
        // Usa document.execCommand para inserir texto (funciona melhor com Lexical)
        ...
        log('Erro ao usar execCommand:', e);
